...
function qa_log_in_external_user($source, $identifier, $fields) {
    ...
    if ($countusers > 1) qa_fatal_error('External login mapped to more than one user'); // should never happen
    ...
    $aggsource = qw_open_login_get_new_source($source, $identifier);
    ...
    if ($countusers) { // user exists so log them in
        ...
        if ($oemail) qw_db_user_login_set($source, $identifier, 'oemail', $oemail);
