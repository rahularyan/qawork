...
function cs_load_addons() {
      ...
      if (!empty($addons)) foreach ($addons as $addon) {
...
function cs_addon_metadata($contents, $fields) {
      ...
      foreach ($fields as $key => $field) if (preg_match('/' . str_replace(' ', '[ \t]*', preg_quote($field, '/')) . ':[ \t]*([^\n\f]*)[\n\f]/i', $contents, $matches)) $metadata[$key] = trim($matches[1]);
...
function widget_opt_update($name, $position, $order, $param, $id = false) {
      ...
      if ($id) {
            qa_db_query_sub(
                    'UPDATE ^ra_widgets SET position = $, widget_order = #, param = $ WHERE id=#', $position, $order, $param, $id
            );
            return $id;
      } else {
            qa_db_query_sub(
                    'INSERT ^ra_widgets (name, position, widget_order, param) VALUES ($, $, #, $)', $name, $position, $order, $param
            );
...
function cs_get_avatar($handle, $size = 40, $html = true) {
      ...
      if (defined('QA_WORDPRESS_INTEGRATE_PATH')) {
            ...
            if (!isset($handle)) {
                  if (qa_opt('avatar_allow_gravatar')) $img_html = qa_get_gravatar_html(qa_get_user_email($userid), $size);
                  else if (qa_opt('avatar_allow_upload') && qa_opt('avatar_default_show') && strlen(qa_opt('avatar_default_blobid'))) $img_html = qa_get_avatar_blob_html(qa_opt('avatar_default_blobid'), qa_opt('avatar_default_width'), qa_opt('avatar_default_height'), $size);
                  else $img_html = '';
            }else {
                  ...
                  } else $img_html = qa_get_user_avatar_html($f['flags'], $f['email'], $handle, $f['avatarblobid'], $size, $size, $size, true);
            }
      }
      if (empty($img_html)) return;

      preg_match('@src="([^"]+)"@', $img_html, $match);
      if ($html) return '<a href="' . qa_path_html('user/' . $handle) . '">' . (!defined('QA_WORDPRESS_INTEGRATE_PATH') ? '<img src="' . $match[1] . '" />' : $img_html) . '</a>';
      elseif (isset($match[1])) return $match[1];
}
...
function cs_get_post_avatar($post, $size = 40, $html = false) {
      ...
      }if (QA_FINAL_EXTERNAL_USERS) $avatar = qa_get_external_avatar_html($post['raw']['userid'], $size, false);
      else $avatar = qa_get_user_avatar_html($post['raw']['flags'], $post['raw']['email'], $post['raw']['handle'], $post['raw']['avatarblobid'], $post['raw']['avatarwidth'], $post['raw']['avatarheight'], $size);
      if ($html) return '<div class="avatar" data-id="' . $post['raw']['userid'] . '" data-handle="' . $post['raw']['handle'] . '">' . $avatar . '</div>';
...
function cs_post_status($item, $description = false) {
      ...
      if (@$item['answer_selected'] || @$item['raw']['selchildid']) {
            ...
            if ($description) $notice .= qa_lang_html('cleanstrap/marked_as_solved');
            ...
            if ($type == 'Q') {
                  ...
                  if ($description) $notice .= qa_lang_html('cleanstrap/marked_as_duplicate');
            }else {
                  $notice = '<span class="post-status closed">' . qa_lang_html('cleanstrap/closed') . '</span>';
                  if ($description) $notice .= qa_lang_html('cleanstrap/marked_as_closed');
            }
      }else {
            $notice = '<span class="post-status open">' . qa_lang_html('cleanstrap/open') . '</span>';
            if ($description) $notice .= qa_lang_html('cleanstrap/marked_as_open');
      }
      return $notice;
}
...
function cs_user_profile($handle, $field = NULL) {
      ...
      if (defined('QA_WORDPRESS_INTEGRATE_PATH')) {
            ...
            if (isset($query[$field])) return $query[$field];
      }
...
function cs_user_badge($handle) {
      if (qa_opt('badge_active')) {
            ...
            $result = qa_db_read_all_values(
                    qa_db_query_sub(
                            'SELECT badge_slug FROM ^userbadges WHERE user_id=#', $userid
                    )
            );
...
function cs_name($handle) {
      ...
      } else $name = cs_user_profile($handle, 'name');
...
function cs_post_link($id) {
      ...
      if ($type == 'A') $id = mysql_result(qa_db_query_sub('SELECT parentid FROM ^posts WHERE postid = "' . $id . '"'), 0);
...
function reset_theme_options() {
      ...
      } else qa_opt('cs_enable_avatar_lists', false);
...
function get_featured_image($postid) {
      ...
      if (!empty($img)) return '<img class="image-preview" id="image-preview" src="' . Q_THEME_URL . '/uploads/' . $img . '" />';
...
function multi_array_key_exists($needle, $haystack) {
      if (isset($haystack) && is_array($haystack)) foreach ($haystack as $key => $value) :
...
function make_array_utf8($arr) {
      foreach ($arr as $key => $value) if (is_array($value)) $arr[$key] = make_array_utf8($value);
            else $arr[$key] = iconv('UTF-8', 'UTF-8//IGNORE', utf8_encode($value));
      return $arr;
}
...
//	Get lists of recent activity in all its forms, plus category information
      ...
      list($questions1, $questions2, $questions3, $questions4) = qa_db_select_with_pending(
              qa_db_qs_selectspec($userid, 'created', 0, $categoryslugs, null, false, false, $qcount), qa_db_recent_a_qs_selectspec($userid, 0, $categoryslugs), qa_db_recent_c_qs_selectspec($userid, 0, $categoryslugs), qa_db_recent_edit_qs_selectspec($userid, 0, $categoryslugs)
      );
...
function cs_followers_list($handle, $limit = 14, $order_by = 'rand') {
      ...
      if ($order_by == 'rand') $order_by = 'ORDER BY RAND()';
...
function cs_event_hook($event, $value = NULL, $callback = NULL, $check = false) {
      ...
      if ($callback !== NULL) {
            ...
            foreach ($events[$event] as $function) {
                  if (is_array($function)) return method_exists($function[0], $function[1]);
      ...
      if (is_array($assets)) {
            ...
            foreach ($assets as $a) {
                  ...
                  if ($parse['host'] == $host_name) {
                        ...
                        if ($css) $styles .= cs_compress_css($content);
                        else $styles .= cs_compress_js($content);
                  }
            }
      }
...
function cs_update_tags_meta($tag, $title, $content) {
      ...
      qa_db_query_sub(
              'REPLACE ^tagmetas (tag, title, content) VALUES ($, $, $)', $tag, $title, $content
      );
