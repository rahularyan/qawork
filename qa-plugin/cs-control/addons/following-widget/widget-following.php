	...
	class cs_following_widget {
		...
		function output_widget($region, $place, $themeobject, $template, $request, $qa_content)
		{
			...
			if(@$themeobject->current_widget['param']['locations']['show_title'])
				$themeobject->output('<h3 class="widget-title">'.qa_lang_sub('cleanstrap/x_following', $account['following']).' <a href="'.qa_path_html('user/'.$handle.'/following').'">'. qa_lang_html('cleanstrap/view_more') .'</a></h3>');
